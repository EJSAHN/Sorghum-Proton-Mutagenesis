...
Inputs:
  - outputs_event_burden/tables/events_long.csv.gz
  - inputs/SbicolorRio_468_v2.1.gene.gff3.gz (or common variants)

Outputs:
  - outputs_feature_enrichment/tables/events_annotated_v2.csv.gz
  - outputs_feature_enrichment/tables/annotation_category_counts.csv
"""
...
def main() -> None:
    ...
    events_path = paths.outputs_event_burden / "tables" / "events_long.csv.gz"
    if not events_path.exists():
        raise FileNotFoundError(f"events_long not found: {events_path}. Run the event-burden step first.")
