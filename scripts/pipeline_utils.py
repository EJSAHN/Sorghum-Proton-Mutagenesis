...
class ProjectPaths:
    ...
    inputs: Path
    outputs: Path
    outputs_event_burden: Path
    outputs_functional: Path

    def ensure(self) -> None:
        self.inputs.mkdir(parents=True, exist_ok=True)
        self.outputs.mkdir(parents=True, exist_ok=True)
        self.outputs_event_burden.mkdir(parents=True, exist_ok=True)
...
def resolve_project_paths(project_root: Optional[str] = None) -> ProjectPaths:
    ...
    paths = ProjectPaths(
        ...
        outputs_event_burden=root / "outputs_event_burden",
        outputs_functional=root / "outputs_feature_enrichment",
    )
    paths.ensure()
    return paths
